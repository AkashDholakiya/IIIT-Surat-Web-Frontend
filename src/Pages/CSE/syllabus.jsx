...
const SyllabusUGCSE = () => {
    ...
    const fetchSyllabus = async () => {
      try {
        ...
        const filteredUGCSE = data.filter(
          item => item.branch === 'UGCSE' && item.active === 1
        );
  ...
  if (syllabusData.length === 0)
    return <p className="p-6 text-center text-gray-700">No syllabus data found for UGCSE.</p>;
  ...
  const getPdfPath = (syltype, filename) => `/CSE/${syltype}/${filename}`;
